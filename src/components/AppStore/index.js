...
class AppStore extends Component {
  ...
  getAppList = searchResult => {
    const {activeTabId} = this.state
    const filteredAppList = searchResult.filter(
  ...
  getSearchResult = () => {
    ...
    const filteredSearchResult = appsList.filter(eachItem =>
  ...
  render() {
    ...
    const filteredAppListResult = this.getAppList(searchResult)
    return (
      <div className="bg-container">
        ...
        <div>
          <ul className="app-list">
            {filteredAppListResult.map(eachItem => (
              <AppItem key={eachItem.appId} appListDetails={eachItem} />
            ))}
          </ul>
        </div>
      </div>
